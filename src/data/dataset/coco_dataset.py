...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def load_item(self, idx):
        ...
        image_id = self.ids[idx]
        # image_id = 550939 tensor([550939], device='cuda:0')
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        # только те объекты что есть в category2label
        #anno = [obj for obj in anno if ('iscrowd' not in obj or obj['iscrowd'] == 0) and (obj['category_id'] in category2label)]
        ...
        if self.return_masks:
            ...
            num_classes = 91
            semantic_mask = torch.zeros((num_classes, h, w), dtype=torch.uint8)

            for segmentation, category_id in zip(segmentations, category_ids):
                # label = category2label[category_id]
                # Декодируем сегментацию объекта
                ...
                # Добавляем маску в соответствующий канал
                semantic_mask[category_id] = semantic_mask[category_id] | mask
                    
            # masks2 = convert_coco_poly_to_mask(segmentations, h, w)
            # masks = semantic_mask #convert_coco_poly_to_mask(segmentations, h, w)
            masks = torch.argmax(semantic_mask, dim=0)
