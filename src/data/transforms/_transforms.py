...
RandomApply = register()(T.RandomApply)
...
class RandomCrop(T.RandomCrop):
    ...
    def __call__(self, *inputs: Any) -> Any:
        if torch.rand(1) >= self.p:
            ...
            inp_w, inp_h = inputs[0][0].size
            ratio = inp_w / inp_h
            random_w = random.randint(inp_w // 3, inp_w)
            random_h = int(random_w / ratio)
            super().__init__([random_h, random_w])
