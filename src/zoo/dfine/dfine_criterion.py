...
class DFINECriterion(nn.Module):
    ...
    def __init__(
        ...
        self.ce_criterion = torch.nn.CrossEntropyLoss(reduction='mean',
                                                       ignore_index=255)
        self.expected_keys = self.generate_expected_keys()
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        # # Проверяем, есть ли FP-предсказания, когда GT пуст
        # pred_boxes = outputs['pred_boxes']
        # if num_boxes == 1 and pred_boxes.numel() > 0:  # Нет объектов, но есть предсказания
        #     false_positive_loss = F.l1_loss(pred_boxes, torch.zeros_like(pred_boxes), reduction='none')
        #     losses['loss_giou_fp'] = false_positive_loss.sum() / (pred_boxes.shape[0] + 1e-6)  # FP штрафуем отдельно
    ...
    def loss_ohem(self, outputs, targets, indices=None, num_boxes=None):
        ...
        for key, sgm_output in outputs.items():  # правильно итерироваться по dict
            ...
            target = torch.nn.functional.interpolate(target.unsqueeze(0).float(), mode="nearest", size=(h, w))
            target = target.long()
    ...
    def generate_expected_keys(self):
        heads = ['loss_bbox', 'loss_giou', 'loss_vfl', 'loss_fgl', 'loss_ddf']
        dn_suffixes = [f'_dn_{i}' for i in range(4)] + ['_dn_pre']
        keys = [head + suffix for head in heads for suffix in dn_suffixes]
        return keys

    def fill_missing_losses(self, losses, expected_keys, device):
        ...
        if "dn_outputs" in outputs:
            ...
            for i, aux_outputs in enumerate(outputs["dn_outputs"]):
                ...
                for loss in self.losses:
                    ...
                    l_dict = self.get_loss(
                        loss, aux_outputs, filtered_targets, indices_dn, dn_num_boxes, **meta
                    )
                    l_dict = {
                        k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict
                    }
